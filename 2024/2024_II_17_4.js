...
const BM_s = segmentLabel(M, H, 0.5, false, false, 0);
const MF_s = segmentLabel(F, G, 0.5, false, false, 0);

const textConfigs = [
  { name: 'txt2', x: M.X()+0.2, y: (H.Y()+M.Y())/2+0.2, content: '2' },
  { name: 'txt3', x: F.X()+0.2, y: (G.Y()+F.Y())/2, content: '5' }
];

const [txt2, txt3] = textConfigs.map(({ x, y, content }) => 
  createText(x, y, content, 12, true)
);

const trapezium = createPolygon([H, M,F,G], {
    fillColor: '#305FB3', fillOpacity:0.6
});

const txt5 = createText((C.X()+H.X())/2, (C.Y()+G.Y())/2, '21cm^2', 12, true)   

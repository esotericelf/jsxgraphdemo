...
const BC_s = segmentLabel(C, F, 0.5, false, false, 0);
const CF_s = segmentLabel(B, C, 0.5, false, false, 0);

const textConfigs = [
  { name: 'txt2', x: (B.X() + C.X()) / 2, y: C.Y() - 1, content: '4' },
  { name: 'txt3', x: (F.X() + C.X()) / 2, y: C.Y() - 1, content: '1' },
  { name: 'txt4', x: C.X(), y: C.Y() - 1, content: ':' }
];
...
const txt5 = createText((C.X()+H.X())/2, (C.Y()+G.Y())/2, '19cm^2', 12, true)   
